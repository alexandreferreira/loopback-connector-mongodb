...
MongoDB.prototype.isObjectIDProperty = function(modelName, prop, value, options) {
  if (
    ...
    if (prop && prop.mongodb && prop.mongodb.dataType === ObjectID) {
      ...
      // else throw new Error(`${value} is not an ObjectID string`);
      return false;
    ...
    var settings = this._models[modelName] && this._models[modelName].settings;
...
function coerceProperties(modelCtor, propValue, propDef, setValue) {
  ...
  if (propDef && propDef.mongodb && propDef.mongodb.dataType) {
    ...
    // ObjectID data type
      ...
      if (!modelCtor.settings.strictObjectIDCoercion) {
    ...
    if (typeof propValue === 'string' && !modelCtor.settings.strictObjectIDCoercion && propValue.match(ObjectIdRegex)) {
      coercedValue = ObjectID(propValue);
      return setValue(coercedValue);
    }
