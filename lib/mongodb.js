...
// Reference for functions that are not bound to the prototype
...
function MongoDB(settings, dataSource) {
  ...
  connectorSettings = this.settings;
...
MongoDB.prototype.collection = function(modelName) {
  ...
  var collectionName = this.collectionName(modelName);
...
MongoDB.prototype.fromDatabase = function(modelName, data) {
  ...
  var modelInfo = this._models[modelName] || this.dataSource.modelBuilder.definitions[modelName];
...
MongoDB.prototype.toDatabase = function(modelName, data) {
  ...
  if (this.settings.enableGeoIndexing !== true) {
    visitAllProperties(data, modelCtor, coerceProperties);
  ...
  function doExecute() {
    ...
    self.notifyObserversAround(
      ...
      function(context, done) {
        ...
        debug('MongoDB: modelName=%s command=%s', modelName, command, args);
...
MongoDB.prototype.coerceId = function(modelName, id, options) {
  ...
  var idProp = self.getPropertyDefinition(modelName, idName);
  ...
  if (idProp && typeof idProp.type === 'function') {
    ...
    const modelCtor = this._models[modelName];
    if (shouldBeObjectID(idProp)) {
      if (isObjectIDProperty(modelCtor, idProp, idValue, options)) {
        idValue = ObjectID(idValue);
      } else {
        throw new Error(`${idValue} is not an ObjectID string`);
      }
    } else if (enableStrictObjectIDCoercion(modelCtor, options)) {
      if (isObjectIDProperty(modelCtor, idProp, idValue, options)) {
        idValue = ObjectID(idValue);
      }
    } else if (isObjectIDProperty(modelCtor, idProp, idValue, options)) {
      idValue = ObjectID(idValue);
    }
  }
  return idValue;
...
MongoDB.prototype.create = function(modelName, data, options, callback) {
  ...
  if (idValue === null) {
    ...
    var oid = self.coerceId(modelName, idValue, options); // Is it an Object ID?c
...
MongoDB.prototype.save = function(modelName, data, options, callback) {
  ...
  var oid = self.coerceId(modelName, idValue, options);
  data._id = oid;
  if (idName !== '_id') {
    delete data[idName];
  }
...
MongoDB.prototype.find = function find(modelName, id, options, callback) {
  ...
  var idName = self.idName(modelName);
  var oid = self.coerceId(modelName, id, options);
...
MongoDB.prototype.updateOrCreate = function updateOrCreate(
  ...
  var id = self.getIdValue(modelName, data);
  var idName = self.idName(modelName);
  var oid = self.coerceId(modelName, id, options);
  delete data[idName];
...
MongoDB.prototype.replaceOrCreate = function(modelName, data, options, cb) {
  ...
  var oid = this.coerceId(modelName, id, options);
...
MongoDB.prototype.buildWhere = function(modelName, where, options) {
  ...
  Object.keys(where).forEach(function(k) {
    ...
    var prop = self.getPropertyDefinition(modelName, propName);
...
MongoDB.prototype.all = function all(modelName, filter, options, callback) {
  ...
  function processResponse(err, cursor) {
    ...
    if (!hasNearFilter(filter.where)) {
      var order = self.buildSort(modelName, filter.order, options);
      cursor.sort(order);
    }
  ...
  if (self.db) {
    ...
    async.each(
      ...
      function(modelName, modelCallback) {
        var indexes = self._models[modelName].settings.indexes || [];
        var indexList = [];
        var index = {};
        var options = {};
        ...
        var properties = self._models[modelName].properties;
...
MongoDB.prototype.automigrate = function(models, cb) {
  ...
  if (self.db) {
    ...
    async.eachSeries(
      models,
      function(modelName, modelCallback) {
        var collectionName = self.collectionName(modelName);
        if (self.debug) {
...
function shouldBeObjectID(propDef) {
  if (!propDef) return false;
  if (propDef.type === ObjectID || propDef.mongodb && propDef.mongodb.dataType === ObjectID) return true;
  else return false;
}
...
function enableStrictObjectIDCoercion(modelCtor, options) {
  ...
  return (settings && settings.strictObjectIDCoercion) ||
    connectorSettings.strictObjectIDCoercion || options &&
...
function isObjectIDProperty(modelCtor, propDef, value, options) {
  if (
    ...
    if (propDef && propDef.mongodb && propDef.mongodb.dataType === ObjectID) {
...
function optimizedFindOrCreate(modelName, filter, data, options, callback) {
  ...
  if (idValue == null) {
    ...
    var oid = self.coerceId(modelName, idValue, options); // Is it an Object ID?
    data._id = oid; // Set it to _id
    if (idName !== '_id') {
      delete data[idName];
    }
  ...
  var sort = self.buildSort(modelName, filter.order, options);
 ...
 */
function coerceProperties(modelCtor, propValue, propDef, setValue) {
  let coercedValue;
